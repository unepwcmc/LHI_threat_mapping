...
InData=["NitrogenBalanceOnLandscape_140Crops","PhosphorusBalanceOnLandscape_140Crops"]
#InData="NitrogenBalanceOnLandscape_140Crops"
...
for i in workspaces:
    ...
    if i==r"C:\Data\WWF\Processing\Global_Processing.gdb":
        ...
        for fc in fclist:
            ...
            #run zonal stats
                        try:
                            arcpy.gp.ZonalStatisticsAsTable_sa(t,"OBJECTID","C:\Data\WWF\Processing\Global_Processing.gdb" + "\\" + raster,i + "\\" + "ZonalStats" + "%s" %raster[0:7],"DATA","MEAN")
                            print "calculating MEAN of the values per grid cell " + "%s" %t + " " + "%s" %raster
                            arcpy.JoinField_management(t,"OBJECTID",i + "\\" + "ZonalStats" + "%s" %raster[0:7],"OBJECTID","MEAN")
                            print "joining MEAN to fishnet grid " + "%s" %t
                            arcpy.AlterField_management(t,"MEAN","Mean_" + "%s" %raster[0:7] + "_kg","Mean "+ "%s" %raster[0:7] + " balance on landscape(kg)")
