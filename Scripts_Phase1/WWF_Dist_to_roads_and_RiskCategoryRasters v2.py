...
#NOTE: v2 adds an extra field to the table for the ranking of threat on an ordinal scale, in addtion to the 'categories' of rad access. This is to bring it closer to the method used for ports
...
# function to calculate access 'risk ranking'
def Access_rank(major_minor_roads):
    major = major_minor_roads[0]
    minor = major_minor_roads[1]
...
#logic for total access cat
    if ((major <= 50000 and major != -1) and (minor <=50000 and minor != -1)):
        return 9
    #logic for v high risk cat
    elif ((major<=50000 and major != -1)and (minor>50000 and minor != -1)):
        return 8
    elif ((major >50000 and major!= -1) and (minor<=50000 and minor != -1)):
        return 7
    #logic for high risk cat
    elif ((major<=50000 and major != -1)and minor==-1):
        return 6
    elif ((minor>50000 and minor!= -1) and (major >50000 and major!= -1)):
        return 5
    #logic for medium risk cat
    elif ((minor>50000 and minor!= -1) and major ==-1):
        return 4
...
#TO CALCULATE ACCESSIBILITY METRICS
                    ...
                    fields = [#("MAJOR_RDS","DOUBLE","7","2","","Major Road Groups(Highway,Secondary,Primary,Tertiary,Unspec)","NULLABLE","NON_REQUIRED",""),
                              ...
                              ("ACCESS_ROAD_RANK","SHORT","","","","Access category scaled from 1 (highest) to 9(no road within search radius)","NULLABLE","NON_REQUIRED","")]
                    if len(arcpy.ListFields(fc[:pos] + "_fishnet_GridToPoint" + "_WGS84","ACCESS_ROAD_RANK"))>0:
                        ...
                        for field in fields:
                            ...
                            print "Adding accessibility rank field" 
#Make a list of tuples with FClass and correspondng attribute value
                ...
                # to know the number of accessiblity categories per PP
                    ...
                    print "%s" %len(list(set(temp_list_access_cats))) + " access categories calculated"
                        ...
                        #==============
                    # logic for  populate the target field 'ACCESS_ROAD_RANK'
                    Access_rank_fields=[]
                    for field in  Allfields:
                        if str(field.name).endswith("_RDS"):
                            if 'MAJOR' in str(field.name) or 'MINOR' in str(field.name):
                                 Access_rank_fields.append(field.name)
                            else:
                                pass
                        else:
                            pass
                    ...
                    for field in  Allfields:
                        if str(field.name)=="ACCESS_ROAD_RANK":
                            Access_rank_fields.append(field.name)       
                        else:
                            pass
                                                
                    if 'ACCESS_ROAD_RANK' not in Access_rank_fields:
                        raise Exception('target field not found')  
                    #sort the list
                    #Access_rank_fields.sort()              

                    with arcpy.da.UpdateCursor(fc[:pos] + "_fishnet_GridToPoint" + "_WGS84", Access_rank_fields) as cursor:
                        for row in cursor:
                                # both input road categories
                            major_minor_roads = row[:-1]
                              # the target access field
                            row[-1] = Access_rank(major_minor_roads)
                            cursor.updateRow(row)
                            
                    print "accessibility risk ranking updated!"
                          #==============
                    # Create categorical 'access risk' raster layer
                    if arcpy.Exists(fc[:pos] + "_road_access_rank"):
                        print str(fc[:pos]).replace("_"," ") + " accessiblity map exists - skipping raster creation"
                    else:
                        arcpy.env.overwriteOutput=True
                        arcpy.FeatureToRaster_conversion(fc[:pos] + "_fishnet_GridToPoint" + "_WGS84","ACCESS_ROAD_RANK",fc[:pos] + "_access_risk_road","0.1")
